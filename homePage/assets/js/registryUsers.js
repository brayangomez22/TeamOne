...
//=============================================
// MOSTRAR EL INPUT PARA EL GRUPO 
//=============================================

function showGroup(evento){

	if(evento == "profesor" || evento == "estudiante"){
		$(".grupo").show("fast");
	}else{
		$(".grupo").hide("fast");
	}
...
function registryUser(){
	...
	if(name != ""){
		...
		if(!expresion.test(name)){
			...
			$("#regUser").before('<div class="alert alert-warning"><strong>ERROR:</strong> No se permiten números ni caracteres especiales</div>')
	...
	if(institution == 0 || institution == ""){
		...
		$("#institution").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Selecciona Una opción</div>')
	...
	if(labor == "labor" || labor == ""){

		$("#labor").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Selecciona Una opción</div>')
